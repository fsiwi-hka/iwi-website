...
function NewsPreviewElement({ title, content, subtitle, image, link }: any) {
    // --- Bulletin-Variante: voller HTML-Content, neutrale Karte, kein Bild/Button ---
    if (content !== undefined) {
        return (
            <div className="w-full white_bg rounded-md p-6 shadow-sm">
                <h4 className="petrol_text font-bold mb-3">{title}</h4>
                <div className="news-content text-gray-700" dangerouslySetInnerHTML={{ __html: content }} />
            </div>
        );
    }

    // --- Original News-Artikel-Variante (Bild + Link + Button) ---
    const hasImage = Boolean(image);

    return (
        <div className="relative w-full">
            {/* Mobile */}
            <a href={link} className="block md:hidden relative rounded-xl overflow-hidden">
                {hasImage ? (
                    ...
                    <div className="primary_blue_bg p-4 text-white">
                        <h4 className="text-lg font-semibold white_text">{title}</h4>
                        <p className="text-sm line-clamp-3 white_text m-0">{subtitle}</p>
                    </div>
                )}
            </a>
                ...
                <div className="hidden md:flex primary_blue_bg text-white rounded-xl flex-col justify-between group/image h-full">
                    <div>
                        ...
                        <div className="p-4">
                            ...
                            <p className="text-white m-0 line-clamp-6">{subtitle}</p>
                        </div>
                    </div>
